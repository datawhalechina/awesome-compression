...
![fp16和bf16](images/fp16和bf16.jpg)
![fp8](images/fp8.jpg)
...
&emsp;&emsp;训练后量化（Post-Training Quantization, PTQ）是指在训练完成后，对模型进行量化，因此也叫做离线量化。根据量化零点  是否为 0，训练后量化分为对称量化和非对称量化，这部分内容已在上述章节进行介绍；根据量化粒度区分，训练后量化又分为逐层量化和逐通道量化以及组量化。同时，我们还要介绍动态量化参数的计算方式；最后我们介绍 round 这个操作带来的影响。
#### 4.4.1 量化粒度
...
&emsp;&emsp;逐通道量化（Channel-wise Quantization就是将数据按照通道维度进行拆分，分别对每一通道的数据进行量化。
#### 4.4.1.4 组量化（Group Quantization）
&emsp;&emsp;组量化是指对每一组向量进行量化。
#### 4.4.2 动态量化参数的计算
#### 4.4.2.1 指数平局
...
#### 4.4.3 Rounding
#### 4.4.3.1 Round-to-Nearest

#### 4.4.3.2 AdaRound

### 4.5 量化感知训练（Quantization-Aware Training）
&emsp;&emsp;量化感知训练（Quantization-Aware Training, QAT）是指在训练过程中，对模型进行量化，并更新模型参数，使得模型在量化后和量化前保持一致。
#### 4.5.1 量化感知训练基本流程
![量化感知训练](images/量化感知训练.jpg)
#### 4.5.2 量化感知训练的反向传播
&emsp;&emsp;量化感知训练的损失函数与普通训练的损失函数类似，只是多了量化误差项。
